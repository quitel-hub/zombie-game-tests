...
#include <SFML/Graphics.hpp>
#include <windows.h> // Для кодування консолі (якщо ще потрібен вивід)
#include "Game.h"
#include "LocalizationManager.h"
...
int main() {
    ...
    L10N.loadLanguage("uk"); // Або "en", або зробити вибір

    // Створюємо вікно SFML
    sf::RenderWindow window(sf::VideoMode(480, 480), "Zombie Survival");
    window.setFramerateLimit(60);

    // Створюємо об'єкт гри, передаючи вікно в конструктор
    Game game(window); // <-- ВИПРАВЛЕНО: Передаємо вікно

    // Запускаємо головний ігровий цикл
    game.runGameLoop(); // <-- ВИПРАВЛЕНО: Викликаємо правильний метод
